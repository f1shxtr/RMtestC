...
void IMU::readSensor()
{
    ...
    float Ax_raw = (float)Ax_int / 32768.0f * range_g * 9.80665f;
    float Ay_raw = (float)Ay_int / 32768.0f * range_g * 9.80665f;
    float Az_raw = (float)Az_int / 32768.0f * range_g * 9.80665f;

    // ======== 一阶低通滤波（加速度） ======== //
    const float ACC_ALPHA = 0.30f;        // alpha 越大，越平滑（越钝）
    static bool acc_first = true;

    if(acc_first){
        accel_sensor_[0] = Ax_raw;
        accel_sensor_[1] = Ay_raw;
        accel_sensor_[2] = Az_raw;
        acc_first = false;
    } else {
        accel_sensor_[0] = ACC_ALPHA * accel_sensor_[0] + (1 - ACC_ALPHA) * Ax_raw;
        accel_sensor_[1] = ACC_ALPHA * accel_sensor_[1] + (1 - ACC_ALPHA) * Ay_raw;
        accel_sensor_[2] = ACC_ALPHA * accel_sensor_[2] + (1 - ACC_ALPHA) * Az_raw;
    }
    ...
    float gx_raw = Gx_int * dps_per_lsb - gyro_bias_[0];
    float gy_raw = Gy_int * dps_per_lsb - gyro_bias_[1];
    float gz_raw = Gz_int * dps_per_lsb - gyro_bias_[2];

    // ======== 一阶低通滤波（陀螺仪） ======== //
    const float GYRO_ALPHA = 0.20f;
    static bool gyro_first = true;

    if(gyro_first){
        gyro_sensor_dps_[0] = gx_raw;
        gyro_sensor_dps_[1] = gy_raw;
        gyro_sensor_dps_[2] = gz_raw;
        gyro_first = false;
    } else {
        gyro_sensor_dps_[0] = GYRO_ALPHA * gyro_sensor_dps_[0] + (1 - GYRO_ALPHA) * gx_raw;
        gyro_sensor_dps_[1] = GYRO_ALPHA * gyro_sensor_dps_[1] + (1 - GYRO_ALPHA) * gy_raw;
        gyro_sensor_dps_[2] = GYRO_ALPHA * gyro_sensor_dps_[2] + (1 - GYRO_ALPHA) * gz_raw;
    }
